...
export function TenantGlobalSearch() {
  ...
  useEffect(() => {
    ...
    employeeApi.employees
      .list({ search: debouncedQuery, pageSize: 8 })
  ...
  function goFirstResult() {
    ...
    if (!first) return;
    window.location.href = ROUTES.TENANT.EMPLOYEES.DETAIL(first.id);
  }
    ...
    <div className="relative w-full max-w-xl" ref={containerRef}>
      <div className="relative flex items-center">
        ...
        <input
          ...
          placeholder={t('tenant.chrome.search.placeholder')}
      ...
      {showPanel && (
        <div className="absolute start-0 top-full z-50 mt-1 max-h-96 w-full overflow-y-auto rounded-xl border border-border-default bg-surface-primary shadow-elevation-3">
          {!hasResults && !isLoading && (
            <p className="px-4 py-3 text-body-sm text-text-secondary">{t('tenant.chrome.search.noResults')}</p>
          )}
          {hasResults && (
            <div>
              <p className="px-4 pb-1 pt-3 text-caption font-semibold uppercase tracking-wide text-text-secondary">
                {t('tenant.chrome.search.employees')}
              </p>
              <ul>
                {results.map((employee) => (
                  <li key={employee.id}>
                    <Link
                      href={ROUTES.TENANT.EMPLOYEES.DETAIL(employee.id)}
                    ...
                    >
                      <div className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded bg-brand-blue-100 text-caption font-semibold text-brand-blue-600">
                        {(employee.displayName[0] ?? 'E').toUpperCase()}
                      </div>
                      <div className="min-w-0">
                        <p className="truncate text-body-sm font-medium text-text-primary">{employee.displayName}</p>
                        <p className="text-caption text-text-secondary">
                          {employee.employeeNumber}
                          {employee.emailWork ? ` · ${employee.emailWork}` : ''}
                        </p>
                      </div>
                    </Link>
                  </li>
