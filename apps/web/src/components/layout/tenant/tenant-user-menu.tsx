...
export function TenantUserMenu({ displayName, initials, onLogout }: TenantUserMenuProps) {
  ...
  const items: Array<{ key: string; href?: string; onClick?: () => void; danger?: boolean }> = [
    { key: 'profile', href: ROUTES.TENANT.SETTINGS_USERS },
    { key: 'mfa', href: ROUTES.AUTH.MFA_SETUP },
    { key: 'settings', href: ROUTES.TENANT.SETTINGS },
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        title={displayName}
      >
        <span className="flex h-8 w-8 items-center justify-center rounded-full bg-brand-teal-500 text-caption font-semibold text-white">
          {initials}
        </span>
      </button>
      {open && (
        ...
        >
          ...
          {items.map((item) => {
            const label = t(`tenant.chrome.userMenu.${item.key}` as Parameters<typeof t>[0]);
            if (item.href) {
              return (
                <Link
                  key={item.key}
