...
import { useCreateDocumentTemplate } from '../../../../../modules/documents/hooks/use-documents';
import { ROUTES } from '../../../../../constants/routes.constants';
import type { CreateDocumentTemplatePayload } from '../../../../../modules/documents/types/documents.types';
...
export default function NewDocumentTemplatePage() {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const payload: CreateDocumentTemplatePayload = {
      ...
      expiryMonths: data.get('expiryMonths')
        ? parseInt(data.get('expiryMonths') as string, 10)
        : undefined,
      isActive: true,
    };
    ...
    createTemplate.mutate(payload, {
      onSuccess: () => {
        router.push(ROUTES.TENANT.DOCUMENTS.TEMPLATES);
      },
    });
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={t('documents.templates.createButton')}
        breadcrumbs={[
          { label: t('tenant.nav.dashboard'), href: ROUTES.TENANT.DASHBOARD },
          { label: t('tenant.nav.documents'), href: ROUTES.TENANT.DOCUMENTS.ROOT },
          { label: t('documents.templates.title'), href: ROUTES.TENANT.DOCUMENTS.TEMPLATES },
          { label: t('documents.templates.createButton') },
        ]}
      />
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-lg">
        ...
        <div>
          <label className="block text-body-sm font-medium text-text-primary mb-1">
            {t('documents.fields.expiryMonths')}
          </label>
          <input
            name="expiryMonths"
            type="number"
        ...
        <div className="flex items-center gap-2">
          <input name="isRequired" type="checkbox" id="isRequired" className="h-4 w-4 rounded border-border-default text-brand-blue-600" />
          <label htmlFor="isRequired" className="text-body-sm text-text-primary">
            {t('documents.fields.isRequired')}
          </label>
        </div>
        ...
        <div className="flex gap-3">
          ...
          >
            {createTemplate.isPending ? t('common.loading') : t('documents.templates.createButton')}
          </button>
          <button
            type="button"
            onClick={() => router.back()}
