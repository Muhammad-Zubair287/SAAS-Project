/**
 * HR Console Scope A permission catalogue seed.
 * Upserts M03–M07 + employee lifecycle permissions and attaches them to HR Manager / HR Admin.
 */
import type { Prisma, PrismaClient } from '@prisma/client';
import { HR_CONSOLE_PERMISSION_CODES } from '../../common/constants/permissions.constants';

type DbClient = PrismaClient | Prisma.TransactionClient;

const HR_ROLE_NAME_PATTERNS = [
  /^hr\s*admin$/i,
  /^hr\s*manager$/i,
];

function splitPermissionCode(code: string): { action: string; resource: string; scope: string } {
  const parts = code.split(':');
  if (parts.length === 3 && parts[0] && parts[1] && parts[2]) {
    return { action: parts[0], resource: parts[1], scope: parts[2] };
  }
  return { action: code, resource: '.', scope: '.' };
}

async function ensurePermission(prisma: DbClient, code: string) {
  const { action, resource, scope } = splitPermissionCode(code);
  return prisma.permission.upsert({
    where: { action_resource_scope: { action, resource, scope } },
    create: {
      action,
      resource,
      scope,
      description: `HR Console: ${code}`,
    },
    update: {},
  });
}

async function attachPermissions(
  prisma: DbClient,
  roleId: string,
  permissionIds: string[],
): Promise<void> {
  for (const permissionId of permissionIds) {
    await prisma.rolePermission.upsert({
      where: { roleId_permissionId: { roleId, permissionId } },
      create: { roleId, permissionId },
      update: {},
    });
  }
}

function isHrRoleName(name: string): boolean {
  return HR_ROLE_NAME_PATTERNS.some((re) => re.test(name.trim()));
}

export async function seedHrConsolePermissions(
  prisma: DbClient,
  options?: { tenantId?: string },
): Promise<{ permissions: string[]; rolesUpdated: number }> {
  const rows = [];
  for (const code of HR_CONSOLE_PERMISSION_CODES) {
    rows.push(await ensurePermission(prisma, code));
  }
  const permissionIds = rows.map((r) => r.id);

  const roles = await prisma.role.findMany({
    where: options?.tenantId ? { tenantId: options.tenantId } : undefined,
    select: { id: true, name: true },
  });

  let rolesUpdated = 0;
  for (const role of roles) {
    if (!isHrRoleName(role.name)) continue;
    await attachPermissions(prisma, role.id, permissionIds);
    rolesUpdated += 1;
  }

  return {
    permissions: [...HR_CONSOLE_PERMISSION_CODES],
    rolesUpdated,
  };
}

export async function ensureHrConsolePermissionsForTenant(
  prisma: DbClient,
  tenantId: string,
): Promise<void> {
  let role = await prisma.role.findFirst({
    where: { tenantId, name: 'HR Manager' },
  });
  if (!role) {
    role = await prisma.role.create({
      data: {
        tenantId,
        name: 'HR Manager',
        description: 'HR operations manager',
        isSystem: true,
      },
    });
  }
  await seedHrConsolePermissions(prisma, { tenantId });
}
